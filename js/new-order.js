(function () {
  ...
  const NewOrder = {
              ...
              <div class="card-body">
                ...
                <div class="form-row mt-2">
                  <div class="form-group">
                    ...
                    <input type="number" class="form-input" id="total-paid" value="0" readonly>
                  </div>
                  <div class="form-group">
                    <label class="form-label">Return to Customer (৳)</label>
                    <input type="number" class="form-input" id="total-change" value="0" readonly style="font-weight:700; color:var(--success);">
                  </div>
                  <div class="form-group">
                    <label class="form-label">Due Amount (৳)</label>
                    <input type="number" class="form-input" id="total-due" value="0" readonly style="font-weight:700; color:var(--danger);">
                  </div>
                </div>
              </div>
            </div>
    ...
    recalculate() {
      ...
      document.getElementById('total-due').value = due.toFixed(2);
    },
    ...
    async placeOrder() {
      ...
      const totalPaid = this.payments.reduce((s, p) => s + p.amount, 0);
      let dueAmount = 0;
      if (totalPaid < grandTotal) dueAmount = grandTotal - totalPaid;
      ...
      const order = {
        ...
        taxAmount,
        grandTotal,
        paidAmount: totalPaid - (totalPaid > grandTotal ? (totalPaid - grandTotal) : 0), // actual paid to clear order
        dueAmount,
        returnedAmount: 0,
        status: 'completed'
      };
      ...
      if (result.success) {
        ...
        if (await H.confirm('Would you like to print the receipt?')) {
          this.printInvoice(order, this.cart);
        }
        POS.Router.navigate('/sales-list');
      } else {
        H.showToast('Failed to place order: ' + result.error, 'error');
      }
    },

    printInvoice(order, cartItems) {
      const H = POS.Helpers;
      let itemsHtml = '';
      cartItems.forEach(item => {
        itemsHtml += `
          <tr>
            <td>${H.esc(item.productName)} ${item.variationName ? `<br><small style="color:#64748b">${H.esc(item.variationName)}</small>` : ''}</td>
      ...
      const html = `
        <div style="text-align: center; margin-bottom: 20px;">
          <h2>🏪 RetailPOS Store</h2>
          <p>123 POS Market Street, Dhaka</p>
          <p>Phone: 01700000000 | Email: support@pos.com</p>
        </div>
        ...
        <div class="totals">
          ...
          ${order.discountAmount > 0 ? `<p>Discount: <strong style="color:#dc2626">-${H.formatCurrency(order.discountAmount)}</strong></p>` : ''}
          ${order.taxAmount > 0 ? `<p>Tax (${order.taxPercent}%): <strong>${H.formatCurrency(order.taxAmount)}</strong></p>` : ''}
          <p class="grand">Grand Total: <span style="color:#059669">${H.formatCurrency(order.grandTotal)}</span></p>
          <hr style="border: none; border-top: 1px dashed #cbd5e1; margin: 10px 0;">
          <p>Paid Amount: <strong>${H.formatCurrency(order.paidAmount)}</strong></p>
          ${order.dueAmount > 0 ? `<p style="color:#dc2626">Due Amount: <strong>${H.formatCurrency(order.dueAmount)}</strong></p>` : ''}
        </div>
        <div style="text-align: center; margin-top: 40px; color: #64748b;">
          <p>Thank you for shopping with us!</p>
          <p>Software Powered by RetailPOS</p>
        </div>
      `;
